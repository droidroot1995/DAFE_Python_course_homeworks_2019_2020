...
''' Создайте функцию, позволяющую находить численное решение уравнения (24) для силы вида;
    ...
    В чем состоит качественное отличие поведения функции от случая невозмущенного движения?'''

def oscillator1(t, z):
    global omega0, gamma, omega, A0
    dy = np.zeros(2)
    
    dy[0] = z[1]
    dy[1] = A0*np.cos(omega*t)- gamma*z[1] - ((omega0)**2)*z[0]
